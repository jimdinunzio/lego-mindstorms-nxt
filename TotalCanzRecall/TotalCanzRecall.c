...
const int LightSensor_FloorTile = 50;
const int LightSensor_BlackTape = 33;
...
const float robotWheelRadius = 27.4;
...
// Search constants
const int captureSlopDistance = 40;
const int NumberOfCansToCollect = 3;
const int halfSweepAngle = 75;
const int BackupDistanceAfterDropOff = -150;
const int DistCloseEnoughForNoCorrection = 500;
...
{
	return round(distance * robotWheelTicksPerRevolution) / (2 * PI * robotWheelRadius);
}
...
void rotateRobot(float angle, int power = rotatePower)
{
	...
	int ticksToGo = round((distanceToGo * robotWheelTicksPerRevolution) / (2 * PI * robotWheelRadius));
	if (ticksToGo < 0)
	{
		ticksToGo = -ticksToGo;
		power = -power;
...
void rotateRobotToHeadingInDeg(float newHeading, int power)
{
...
void goToDestination(Vector2d dest)
{
	...
	while(doCorrection && !gStopGoingHome);
}
...
bool findClosestTarget(float& distance, float& angle)
{
	...
	while (!found)
	{
		...
		semaphoreLock(findObjectResultSemaphore);
		writeDebugStreamLine("Main: result ready, object distance is %d, angle is %d",
				gTargetDistance, myRadiansToDegrees(gTargetAngle));
		semaphoreUnlock(findObjectResultSemaphore);
...
bool captureTarget(float distanceToTarget, float angleToTarget)
{
	...
	while (nMotorRunState[Master] != runStateIdle) //while Right motor is not in idle
	{
		// if we are < 10 cm have run into object
		if (SensorValue[Distance] < 12)
		{
			break;
		}
	}
	motor[Master] = 0;
	nSyncedMotors = synchNone;
	return (SensorValue[Distance] < 10);
}

task stopWhenWhitePaper()
{
	while (1)
	{
		if (SensorValue[Light] < LightSensor_FloorTile_BlackTape_Threshhold)
		{
			...
			while (1)
			{
				if (SensorValue[Light] > LightSensor_BlackTape_WhitePaper_Threshhold)
				{
					writeDebugStreamLine("White Paper detected.");
					// stop robot
					gStopGoingHome = true;
					motor[Master] = 0;
					return;
...
task main()
{
	...
	while (1)
	{
		...
		// Turn robot to heading of 45 degrees to roughly face center of "room"
		...
		if (findClosestTarget(distToClosestTarget, angleToClosestTarget))
		{
			...
			{
				writeDebugStreamLine("distance to target is less than threshold dist, attempting to capture it");
				...
				if (captured)
				{
					...
					startTask(stopWhenWhitePaper);
					gStopGoingHome = false;
					goToDestination(home);
					stopTask(stopWhenWhitePaper);
					wait1Msec(50);
			...
			{
				writeDebugStreamLine("distance to target is > threshold, driving half way to it and scanning again.");
				...
				driveRobotStraightInMm(max(distToClosestTarget / 2.0, DistCloseEnoughForNoCorrection));
			}
		}
		else // found no target so go forward some on 45 degree heading
		{
			rotateRobotToHeadingInDeg(HeadingToPointBeforeSearch, rotatePower);
